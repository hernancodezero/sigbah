...
var listaCorrelativo = new Object();
...
var tbl_correlativo= $('#tbl_correlativo');
...
var frm_chofer = $('#frm_chofer');
var frm_correlativo = $('#frm_correlativo');
...
//		}
	...
	$('#sel_tablas_generales').change(function() {
		...
		if(tabla=="BAH_MAE_ALMACEN_EXTERNO"){
			...
			cod = codigo;
		}else if(tabla=="BAH_D_CORRELATIVO"){
	...
	$('#href_nuevo').click(function(e) {
		...
		if(indicador=="BAH_MAE_ALMACEN"){
		...
		}else if(indicador=="BAH_MAE_CHOFER"){
			$('#h4_tit_chofer').html('Nuevo Chofer');
			cargarEmpresa('1',null);
			$('#hid_id_chofer').val('');
			...
			$('#div_chofer').modal('show');
		}else if(indicador=="BAH_D_CORRELATIVO"){
			$('#h4_tit_correlativo').html('Nuevo Correlativo Guía de Remisión');
					
			$('#hid_id_correlativo').val('');
			$('#hid_flag_activo_corr').val('1');
//			$('#hid_activa_formato_corr').val(obj.activaFormato);
			$('#hid_cod_tip_corr').val('');
//			$('#hid_id_almacen_corr').val('');

			$('#sel_est_correlativo').val('1');
			$('#sel_est_correlativo').prop('disabled', true);
//			$('#txt_nom_alm_corr').val('');
			$('#txt_nom_alm_corr').prop('disabled', true);
			$('#txt_nro_serie_corr').val('');
			$('#txt_nro_inicio_corr').val('');
			$('#txt_nro_final_corr').val('');
			$('#txt_nro_actual_corr').val('');
			$('#txt_nro_aut_corr').val('');
			$('#txt_nom_imp_corr').val('');
			$('#txt_ruc_imp_corr').val('');
			$('#txt_fecha_ini_corr').val('');
	
			$('#div_correlativo').modal('show');
		}
...
//					$('#sel_distrito_per_ext').val(obj.codUbigeo);
				...
				$('#h4_tit_chofer').html('Actualizar Chofer');
				
				var obj = listaChofer[indices[0]];
				
				$('#hid_id_chofer').val(obj.idChofer);
				cargarEmpresa('1', obj.idEmpresaTransporte);
//				$('#sel_emp_transporte').val();
				$('#txt_nom_cho').val(obj.nombreChofer);
				$('#txt_dni_cho').val(obj.nroDni);
				$('#txt_bre_cho').val(obj.nroBrevete);
				$('#sel_est_cho').val(obj.flagActivo);
				
				$('#div_chofer').modal('show');
			}else if(indicador=="BAH_D_CORRELATIVO"){
				$('#h4_tit_correlativo').html('Actualizar Correlativo Guía de Remisión');
				
				var obj = listaCorrelativo[indices[0]];
				
				$('#hid_id_correlativo').val(obj.idCorrelativo);
				$('#hid_flag_activo_corr').val(obj.flagActivo);
				$('#hid_activa_formato_corr').val(obj.activaFormato);
				$('#hid_cod_tip_corr').val(obj.codTipo);
				$('#hid_id_almacen_corr').val(obj.idAlmacen);
//				if(!esnulo(obj.activarFormato)){
//					$('input[name=rb_correlativo][value="'+obj.activarFormato+'"]').prop('checked', true);
//				}else{
//					$('input[name=rb_correlativo][value="1"]').prop('checked', true);
//				}
				
				$('#sel_est_correlativo').val(obj.flagActivo);
				$('#sel_est_correlativo').prop('disabled', false);
				$('#txt_nom_alm_corr').val(obj.nombreAlmacen);
				$('#txt_nom_alm_corr').prop('disabled', true);
				$('#txt_nro_serie_corr').val(obj.nroSerie);
				$('#txt_nro_inicio_corr').val(obj.nroInicio);
				$('#txt_nro_final_corr').val(obj.nroFin);
				$('#txt_nro_actual_corr').val(obj.nroActual);
				$('#txt_nro_aut_corr').val(obj.nroAutorizacion);
				$('#txt_nom_imp_corr').val(obj.nombreImprenta);
				$('#txt_ruc_imp_corr').val(obj.rucImprenta);
				$('#txt_fecha_ini_corr').val(obj.fechaInicio);
				console.log("FECHA "+obj.fechaInicio);
				$('#div_correlativo').modal('show');
			}
...
//			$('#div_imp_pdf').modal('hide');
	...
	$('#btn_gra_cho').click(function(e) {
		e.preventDefault();
		
		var bootstrapValidator = frm_chofer.data('bootstrapValidator');
		bootstrapValidator.validate();
		if (bootstrapValidator.isValid()) {
			loadding(true);
			
			var params = { 
				idChofer :	$('#hid_id_chofer').val(),
				idEmpresaTransporte : $('#sel_emp_transporte').val(),
				nombreChofer : $('#txt_nom_cho').val(),
				nroDni : $('#txt_dni_cho').val(),
				nroBrevete : $('#txt_bre_cho').val(),
				flagActivo : $('#sel_est_cho').val()
			};
			loadding(true);
			consultarAjax('POST', '/administracion/tablas-generales/grabarChofer', params, function(respuesta) {
				if(respuesta.codigoRespuesta == NOTIFICACION_VALIDACION){
					loadding(false);
					addErrorMessage(null, respuesta.mensajeRespuesta);
				}else{
					$('#div_chofer').modal('hide');
					if (respuesta.codigoRespuesta == NOTIFICACION_ERROR) {
						loadding(false);
						addErrorMessage(null, respuesta.mensajeRespuesta);
					} else {
					...
					frm_chofer.data('bootstrapValidator').resetForm();
				}
			});
		}
	});
	
	$('#btn_gra_cor').click(function(e) {
		e.preventDefault();
		
		var bootstrapValidator = frm_correlativo.data('bootstrapValidator');
		bootstrapValidator.validate();
		if (bootstrapValidator.isValid()) {
			loadding(true);
			var flag=null;
//			if($('input[name=rb_correlativo]:checked').val()=='1'){
//				flag='1';
//			}else{
//				if($('input[name=rb_correlativo]:checked').val()=='0'){
//					flag='0';
//				}
//			}
			console.log("ID "+$('#hid_id_correlativo').val());
			if(!esnulo($('#hid_id_correlativo').val())){
				flag=$('#sel_est_correlativo').val();
			}else{
				flag='1';
			}
			...
			var params = { 
				idCorrelativo :	$('#hid_id_correlativo').val(),
				idAlmacen : $('#hid_id_almacen_corr').val(),
				nroSerie: $('#txt_nro_serie_corr').val(),
				nroInicio : $('#txt_nro_inicio_corr').val(),
				nroFin : $('#txt_nro_final_corr').val(),
				nroActual : $('#txt_nro_actual_corr').val(),
				flagActivo : flag,
				codTipo : '1',
				//activarFormato : flag,
				nombreImprenta : $('#txt_nom_imp_corr').val(),
				rucImprenta : $('#txt_ruc_imp_corr').val(),
				nroAutorizacion : $('#txt_nro_aut_corr').val(),
				fechaInicio : $('#txt_fecha_ini_corr').val(),
				control : 'U'
				
			};
			loadding(true);
			consultarAjax('POST', '/administracion/tablas-generales/grabarCorrelativo', params, function(respuesta) {
				if(respuesta.codigoRespuesta == NOTIFICACION_VIGENCIA){
					loadding(false);
					addWarnMessage(null, respuesta.mensajeRespuesta);
				}else if(respuesta.codigoRespuesta == NOTIFICACION_VALIDACION){
					loadding(false);
					addErrorMessage(null, respuesta.mensajeRespuesta);
				}else{
					$('#div_correlativo').modal('hide');
					if (respuesta.codigoRespuesta == NOTIFICACION_ERROR) {
						loadding(false);
						addErrorMessage(null, respuesta.mensajeRespuesta);
					} else {
					...
					frm_correlativo.data('bootstrapValidator').resetForm();
				}
			});
		}
	});
	...
	$('#txt_nro_serie_corr').change(function() {
		var cantidad = $(this).val();
		$('#txt_nro_serie_corr').val(zeroFill(cantidad, 3));
		
	});
	
	$('#txt_nro_inicio_corr').change(function() {
		var cantidad = $(this).val();
		$('#txt_nro_inicio_corr').val(zeroFill(cantidad, 6));
		validarCamposCorrelativo();
	});
	
	$('#txt_nro_final_corr').change(function() {
		var cantidad = $(this).val();
		$('#txt_nro_final_corr').val(zeroFill(cantidad, 6));
		validarCamposCorrelativo();
	});
	
	$('#txt_nro_actual_corr').change(function() {
		var cantidad = $(this).val();
		$('#txt_nro_actual_corr').val(zeroFill(cantidad, 6));
		validarCamposCorrelativo();
	});
...
function inicializarDatos() {
	...
	$('#li_tablas_ddi').closest('li').children('a');
	limpiarTablas();
	
	if (codigoRespuesta == NOTIFICACION_ERROR) {
		addErrorMessage(null, mensajeRespuesta);
	} else {
		console.log(dataAlmacen.nombreAlmacen);
		console.log(dataAlmacen.idAlmacen);
		$('#hid_id_almacen_corr').val(dataAlmacen.idAlmacen);
		$('#txt_nom_alm_corr').val(dataAlmacen.nombreAlmacen);
	}
}
...
function cargarEmpresa(codigo,dato){
	if(codigo=='1'){
...
function listarCorrelativo(){
	...
	var menu=null;
		...
		loadding(true);
		consultarAjax('GET', '/administracion/tablas-generales/listarCorrelativo', params, function(respuesta) {
			if (respuesta.codigoRespuesta == NOTIFICACION_ERROR) {
				addErrorMessage(null, respuesta.mensajeRespuesta);
			} else {

				indicador="BAH_D_CORRELATIVO";
				mostrarTablaCorrelativo(respuesta);
				nombreTabla=tbl_correlativo;
				tablaExcel ="tbl_correlativo";
				$('#div_tbl_correlativo').show();
...
function mostrarTablaCorrelativo(respuesta) {

	tbl_correlativo.dataTable().fnDestroy();
	
	tbl_correlativo.dataTable({
		data : respuesta,
		columns : [ {
			data : 'idCorrelativo',
			sClass : 'opc-center',
...
function validarCamposCorrelativo(){
	frm_correlativo.bootstrapValidator('revalidateField', 'txt_nro_inicio_corr');
	frm_correlativo.bootstrapValidator('revalidateField', 'txt_nro_final_corr');
	frm_correlativo.bootstrapValidator('revalidateField', 'txt_nro_actual_corr');
}
