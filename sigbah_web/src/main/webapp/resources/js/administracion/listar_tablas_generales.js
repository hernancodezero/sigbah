...
//		}
	...
	$('#sel_tablas_generales').change(function() {
		...
		if(tabla=="BAH_MAE_ALMACEN_EXTERNO"){
			...
			if(tabla=="BAH_MAE_DISTRITO_INEI"){
				$('#div_busqueda_poblacion_inei').show();
				nombreTabla=tbl_distrito_inei;
				tablaExcel = "tbl_distrito_inei";
				$('#href_req_subir').show();
				$('#href_req_descargar').show();
				$('#href_editar').hide();
				$('#href_nuevo').hide();
				
				cod = codigo;
				console.log("1 "+tablaExcel);
				console.log("2 "+cod);
			}else{
				$('#div_busqueda_alm_ext').hide();
				$('#div_busqueda_poblacion_inei').hide();
	...
	$('#href_req_subir').click(function(e) {
		if(indicador=="BAH_MAE_DISTRITO_INEI"){
			$('#h4_tit_poblacion_inei').html('Subir Archivo con la Población INEI');
...
//			$('input[name=rb_tie_ate_gobierno]').prop('checked', false);
			$('input[name=rb_tie_ate_gobierno][value="'+flagTipo+'"]').prop('checked', true);
			console.log("TIPO L O R : "+flagTipo);
			if(flagTipo=='R'){
				...
				$('#sel_departamento').prop('disabled', true);
				$('#sel_departamento').val($('#sel_departamento_bus').val());
				cargarProvincia($('#sel_departamento_bus').val(), $('#sel_provincia_bus').val());					
				cargarDistrito($('#sel_provincia_bus').val(), '');
//				$('#sel_provincia_per_ext').val(obj.codProvincia);
//				$('#sel_distrito_per_ext').val(obj.codUbigeo);
				$('#sel_gore').val('');
				$('#div_gore_destino').hide();
				$('#div_ubi_destino1').show();
...
//			$('#div_ubi_destino').hide();
		...
		}else if(indicador=="BAH_MAE_MARCA"){
			$('#h4_tit_marca').html('Nueva Marca');
			$('#hid_id_marca').val('');
			$('#txt_nom_marca').val('');
			$('#div_marca').modal('show');
		}
...
//					$('#sel_distrito_per_ext').val(obj.codUbigeo);
				...
				var obj = listaTipDocumento[indices[0]];
				...
				$('#sel_est_uni').val(obj.idFlagActivo);
				$('#div_unidad_medida').modal('show');
			}else if(indicador=="BAH_MAE_MARCA"){
				$('#h4_tit_marca').html('Actualizar Marca');
				var obj = listaMarca[indices[0]];
				$('#hid_id_marca').val(obj.idMarca);
				$('#txt_nom_marca').val(obj.nombreMarca);
				$('#div_marca').modal('show');
			}
...
//			
	...
	$('#href_exp_excel').click(function(e) {
		...
		if(tablaExcel=="tbl_distrito_inei"){
			var row = $('#tbl_distrito_inei > tbody > tr').length;
			var empty = null;
			...
			loadding(true);
			var codanio = $('#sel_anio_inei').val();
			var coddepa = $('#sel_departamento_inei').val();
			var url = VAR_CONTEXT + '/administracion/tablas-generales/exportarExcelTablaInei/';
			url += verificaParametro(codanio) + '/';
			url += verificaParametro(coddepa);
	...
	$('#href_req_descargar').click(function(e) {
		e.preventDefault();
			...
			var codanio = $('#sel_anio_inei').val();
			var coddepa = $('#sel_departamento_inei').val();
			var url = VAR_CONTEXT + '/administracion/tablas-generales/exportarExcelInei/';
			url += verificaParametro(codanio) + '/';
			url += verificaParametro(coddepa);
...
//			$('#div_imp_pdf').modal('hide');
	...
	$('#btn_buscar_poblacion_inei').click(function(e) {
		buscarPoblacionInei();
	});
...
//				nombreSecundario : $('#txt_sec_alm').val()
	...
	$('#btn_gra_per_inei').click(function(e) {
		e.preventDefault();
		var anio= $('#sel_anio_inei_subir').val();
		var nro="";
		var codi="";
		var mens="";
		swal({
			  title: 'Está seguro?',
			  text: 'Está seguro que desea subir esta data?',
			  type: 'warning',
			  ...
			  cancelButtonText: 'No',
				...
				if (cod_ind_alfresco == '1' || cod_ind_alfresco == '2') { // Archivo cargado
					...
					consultarAjaxFile('POST', '/common/archivo/cargarArchivoExcel/'+anio, formData, function(resArchivo) {
						console.log(resArchivo);
						
						if (!esnulo(resArchivo)) {
							var arr = resArchivo.split('_');
							nro=arr[0];
							codi = arr[1];
							mens = arr[2];
						}
					
						console.log("codigo error: "+NOTIFICACION_ERROR);
						if (codi == "02") {
							swal(
								    'No Registrado!',
								    mens,
...
//	});
	...
	$('#btn_gra_tip_doc').click(function(e) {
		...
		if (bootstrapValidator.isValid()) {
			...
			var params = { 
				idTipDocumento :	$('#hid_id_tipo_documento').val(),
	...
	$('#btn_gra_marca').click(function(e) {
		e.preventDefault();
		
		var bootstrapValidator = frm_marca.data('bootstrapValidator');
		bootstrapValidator.validate();
		if (bootstrapValidator.isValid()) {
			loadding(true);
			
			var params = { 
				idMarca :	$('#hid_id_marca').val(),
				nombreMarca : $('#txt_nom_marca').val(),

			};
			loadding(true);
			consultarAjax('POST', '/administracion/tablas-generales/grabarMarca', params, function(respuesta) {
				if(respuesta.codigoRespuesta == NOTIFICACION_VALIDACION){
					loadding(false);
					addErrorMessage(null, respuesta.mensajeRespuesta);
				}else{
					$('#div_marca').modal('hide');
					if (respuesta.codigoRespuesta == NOTIFICACION_ERROR) {
						loadding(false);
						addErrorMessage(null, respuesta.mensajeRespuesta);
					} else {
					...
					frm_marca.data('bootstrapValidator').resetForm();
				}
			});
		}
	});
...
function inicializarDatos() {
	...
	$('#li_tablas_generales').closest('li').children('a');
	limpiarTablas();
...
function mostrarTablaMarca(respuesta) {
	...
	if (!esnulo(codigo)) {
		...
		consultarAjax('GET', '/administracion/tablas-generales/listarTabla', params, function(respuesta) {
			if (respuesta.codigoRespuesta == NOTIFICACION_ERROR) {
				...
				}else if(tabla=='BAH_MAE_MARCA'){
					indicador="BAH_MAE_MARCA";
					mostrarTablaMarca(respuesta);
					nombreTabla=tbl_marca;
					tablaExcel = "tbl_marca";
					$('#div_tbl_marca').show();
				}
...
function buscarPoblacionInei(){
	...
	if (!esnulo(cod)) {						
		var params = { 
			vcodigoParam2 : $('#sel_anio_inei').val(),
			vcodigoParam3 : $('#sel_departamento_inei').val()
			
		};			
		loadding(true);
		consultarAjax('GET', '/administracion/tablas-generales/listarTablaPoblacionInei', params, function(respuesta) {
			if (respuesta.codigoRespuesta == NOTIFICACION_ERROR) {
				addErrorMessage(null, respuesta.mensajeRespuesta);
			} else {
