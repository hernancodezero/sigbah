...
var tbl_repo_kardex = $('#tbl_repo_kardex');
var listaRepoKardexCache = new Object();
	...
	$('#btn_buscar').click(function(e) {
		...
		if (bootstrapValidatorRepAlmacen.isValid() && bootstrapValidatorTipReporte.isValid()) {
			...
			var params = { 
				...
				idProducto : verificaParametroInt(codigoProducto),
			...
			consultarAjax('GET', '/gestion-almacenes/reporte-almacen/exportarRepoKardex', params, function(respuesta) {
			
				if (respuesta.codigoRespuesta == NOTIFICACION_ERROR) {
					addErrorMessage(null, respuesta.mensajeRespuesta);
				} else {
					listarRepoKardex(respuesta);
				}
				loadding(false);
			});
		}
...
function inicializarDatos() {
	...
	$('#ul_alm_reportes').css('display', 'block');
	$('#li_alm_rep_kar').attr('class', 'active');
	$('#li_alm_rep_kar').closest('li').children('a');
...
function listarRepoKardex(respuesta) {

	tbl_repo_kardex.dataTable().fnDestroy();
	
	tbl_repo_kardex.dataTable({
		data : respuesta,
		columns : [ {	
			data : 'idRegistro',
	...
	listaRepoKardexCache = respuesta;
