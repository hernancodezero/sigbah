...
$(document).ready(function() {
	...
	$('#href_pro_nuevo').click(function(e) {
		...
		cargarProductos("I",null);
		$('#sel_producto').prop('disabled',false);
		$('#hid_cod_producto').val('');
		$('#txt_observaciones_producto').val('');
	...
	$('#href_pro_editar').click(function(e) {
		...
		if (indices.length == 0) {
			...
			var val_producto = obj.idProducto+'_'+obj.codigoProducto+'_'+obj.nombreUnidad+'_'+obj.nombreEnvase;
			//var val_producto = obj.idProducto+'_'+obj.codigoProducto+'_'+obj.nombreUnidad+'_'+obj.nombreEnvase;
			...
			cargarProductos("U",val_producto);
			cargarLoteProducto(obj.idProducto, obj.nroLote+'_'+obj.cantidadStock+'_'+obj.precioUnitario, "U");
			//$('#sel_producto').prop('disabled',true);
			$('#txt_pro_can_stock').val(obj.cantidadStock);
			$('#txt_pro_pre_unitario').val(obj.precioUnitario);
			$('#txt_observaciones_producto').val(obj.observacion);
			$('#div_det_productos').modal('show');
		}
...
//			});
	...
	$('#btn_gra_producto').click(function(e) {
		...
		if (bootstrapValidator.isValid()) {
			...
			var params = { 
				...
				observacion : $('#txt_observaciones_producto').val()
			};
	...
	$('#sel_producto').change(function() {
		...
		if (!esnulo(codigo)) {
			...
			cargarLoteProducto(arr[0], null, "I");
		} else {
			$('#txt_pro_codigo').val('');
			$('#txt_pro_uni_medida').val('');
			$('#txt_pro_envase').val('');
...
//		});
	...
	$('#href_aju_editar').click(function(e) {
		...
		if (indices.length == 0) {
			...
			$('#txt_observaciones_ajuste').val(obj.observacion.replace(/<<E>>/g, "\n").replace(/<<D>>/g, "\"").replace(/<<S>>/g, "\'"));
	...
	$('#btn_gra_ajuste').click(function(e) {
		...
		if (bootstrapValidator.isValid()) {
			var params = { 
				...
				observacion : $('#txt_observaciones_ajuste').val()
			};
...
function cargarLoteProducto(idProducto, nroLote, control) {
	var params = {
		idDdi : cartillaInventario.idDdi,
		idAlmacen : cartillaInventario.idAlmacen,
		idProducto : idProducto,
		idCartilla : cartillaInventario.idCartilla,
		control : control
	};			
	console.log("IDCARTILLA: "+cartillaInventario.idCartilla);
	loadding(true);
...
function cargarProductos(control, dato) {
	var params = {
		control : control,
		idCartilla : $('#hid_cod_cartilla').val()
	};			
	console.log("IDCARTILLA: "+cartillaInventario.idCartilla);
	loadding(true);
	consultarAjax('GET', '/gestion-almacenes/cartilla-inventario/listarProductos', params, function(respuesta) {
		if (respuesta.codigoRespuesta == NOTIFICACION_ERROR) {
			addErrorMessage(null, respuesta.mensajeRespuesta);
		} else {
			var options = '<option value="">Seleccione</option>';
	        $.each(respuesta, function(i, item) {
	            options += '<option value="'+item.idProducto+'_'+item.codigoProducto+'_'+item.nombreUnidad+'_'+item.nombreEnvase+'">'+item.nombreProducto+'</option>';
	        });
	        $('#sel_producto').html(options);
	        if (dato != null) {
	        	console.log("DATO: "+dato);
	        	$('#sel_producto').val(dato);
	        }
