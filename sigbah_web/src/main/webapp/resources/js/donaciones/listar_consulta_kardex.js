...
e.preventDefault();
		...
		var row = $('#tbl_stock_todos > tbody > tr').length;
		var empty = null;
	    ...
	    tab = document.getElementById('tbl_stock_todos'); // id of table
	...
	$('#href_imprimir').click(function(e) {
		...
		var arr = '';
		if(!esnulo(idProducto)){
			arr = idProducto.split('_');
		}
		idProducto = arr[0];
		loadding(true);
		var url = VAR_CONTEXT + '/donaciones/consulta/exportarPdfKardex/'+codigoAnio+'/'+codigoMes+'/'+idProducto;
...
function inicializarDatos() {
  	...
  	$('#li_consultas_kardex').addClass('active');
	$('#ul_donaciones').css('display', 'block');
	$('#ul_don_inventarios').css('display', 'block');
	$('#li_consultas_kardex').attr('class', 'active');
	$('#li_consultas_kardex').closest('li').children('a');
...
function listarTablaStock(){
	...
	console.log(arr[0]+ codigoAnio+ codigoMes);
		var params = { 
		...
		consultarAjax('GET', '/donaciones/consulta/listarKardex', params, function(respuesta) {
